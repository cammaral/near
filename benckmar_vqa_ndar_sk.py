...
def run_plain_vqa(
    *,
    ...
    for block_idx in range(K):
        ...
        sub = train_df.iloc[:epoch_checkpoint]
        best_epoch = int(sub.loc[sub["loss"].idxmin(), "epoch"])
        best_thetas.append(best_epoch)
        theta_checkpoint = None
        # replay training from stored theta_final is unavailable from optimize(), so we use
        # the best final theta found over the first checkpoint epochs by rerunning short optimize.
        # This keeps the script standalone and faithful enough for checkpoint comparison.
        ...
        theta_checkpoints.append(theta_checkpoint)
...
def plot_fig1_like(
    ...
    # Overlay cumulative-best lines for baselines
    style_map = {
...
def plot_metric_evolution(
    ...
    for method, g in df_all.groupby("method", sort=False):
        # faint instance curves
        for (_, _, inst), gi in g.groupby(["noise", "instance_idx", "method"]):
            gi = gi.sort_values("step")
            ax.plot(gi["step"], gi[metric], alpha=0.15, lw=1)

        agg = g.groupby("step", as_index=False)[metric].agg(["mean", "std"]).reset_index()
        ax.plot(
            agg["step"],
            agg["mean"],
            lw=2.8,
...
def plot_all_noises_combined(
    ...
    for (noise_name, method), g in df_all.groupby(["noise", "method"], sort=False):
        agg = g.groupby("step", as_index=False)[metric].mean()
        ax.plot(
            agg["step"],
            agg[metric],
            marker="o",
...
def main() -> None:
    ...
    for noise in NOISE_SWEEP:
        ...
        for instance_idx in range(cfg["n_instances"]):
            print(f"\n--- instance {instance_idx + 1}/{cfg['n_instances']} | noise={noise.name} ---")
            ...
            H0_sparse, H0_constant_term, H0_zz_terms = build_maxcut_hamiltonian(cfg["n_qubits"], edges)
            ...
            df_instance = pd.concat([df_plain, df_ndar, df_rand, df_noopt], ignore_index=True)
            df_instance["instance_idx"] = instance_idx
            df_instance["noise"] = noise.name
            df_instance.to_csv(instance_dir / "all_methods_summary.csv", index=False)

            if instance_idx == cfg["selected_instance_for_fig1"]:
        ...
        plot_metric_evolution(
            ...
            title=f"AR evolution across methods | noise={noise.name}",
        )
        plot_metric_evolution(
            df_noise,
            ...
            title=f"Energy evolution across methods | noise={noise.name}",
        )
